...
//! Funciona con la escena youboTorqueMode.tt
//! en esta ultima version se cambio toda la parte de conexion con vrep a otro archivo llamado vrepInit.cpp el cual esta ligado a este main.cpp mediante el header vrepInit.h
...
class KUKA{
 ...
 private:
    ...
    double l1,l2,l3,l4,l5,l6,l7, t0, tEnd, delta,delta2, Kp, Kd,Kwh,Krot, Num_Iter, delay;
    ...
    VectorXd mass{Dof}, h{Dof}, qS{Dof},qI{Dof}, qM{Dof}, QS{Dof},QI{Dof},fr{Dof},D{Dof},Q{Dof}, QDot{Dof}, QDDot{Dof},
             ...
             xOpt1{nV},xOpt2{nV2}, POSE{7},err{6}, bA{6},tkd{6},tkp{6},
    ...
    MatrixXd H{Dof,Dof}, Ja{6,Dof}, dJa{6,Dof}, Ag{6,6}, Jg{6, Dof}, dJg{6, Dof}, dJxInv{6, 6}, JxInv{6, 6}, Jeps{3,4},
             Jth{3,4}, JthInv{4,3}, Fricc{Dof,Dof}, eye8{8,8}, zeros68{6,8}, zeros8{8,8}, G{nV,nV},
             G2{nV2,nV2}, zeros19_3{19,3},zeros38_6{38,6},kd{6,6},kp{6,6},
  ...
  public:
    ...
    KUKA () { //! Constructor. Parameters of the KUKAyouBot
        ...
        *******************************************************************************/
        ...
        tkd << 0.8, 0.8, 0.8, 0.85, 0.85, 0.85;
        Kwh = 9;
        Krot= 4.8;
        delta = 0.01;
        delta2 = 0.1;
    ...
    void QPsolver()
        {
            ...
            int wheel_rl, wheel_rr,wheel_fl,wheel_fr,joint1, joint2, joint3, joint4, joint5,
                wheel_rl2, wheel_rr2,wheel_fl2,wheel_fr2,joint1_2, joint2_2, joint3_2, joint4_2, joint5_2,
                 baseX,Ox,Ox2, baseX2,youBot_Tip,youBot_Tip2, leftTarget, rightTarget, WorldFrame2;
            float prismaticos[3],baseRot[3],tip[3],desired[3],desired2[3],prismaticos2[3],baseRot2[3],tip2[3];
            ...
            if (valido1 != 0 || valido2 != 0 || valido3 != 0 || valido4 != 0 || valido5 != 0 || valido6 != 0 || valido7 != 0  || valido8 != 0 || valido9 != 0 || valido10 != 0 || valido11 != 0 || valido12 != 0 || valido13 != 0 || valido14 != 0 || valido15 != 0 || valido16 != 0 || valido17 != 0 || valido18 != 0 || valido81 != 0 || valido19 != 0 ||  valido21 != 0 || valido22 != 0 || valido23 != 0 || valido24 != 0 || valido25 != 0 || valido26 != 0|| valido27 != 0)
            {
                std::cout << "ERROR: Unable to connect with V-rep \n";
            ...
            for(int j=0; j<=300000; j++){
             ...
             posD2<< desired2[0], desired2[1], desired2[2];
                */
             resp = vrepCOM.connect(Q,Q2,QDot,Q2Dot);
             Q(0) = resp(0);
             Q(1) = resp(1);
             Q(2) = resp(2);
             Q2(0) = resp(3);
             Q2(1) = resp(4);
             Q2(2) = resp(5);
             pos<< resp(6), resp(7), resp(8);
             pos2<< resp(9), resp(10), resp(11);
             posD<< resp(12), resp(13), resp(14);
             posD2<< resp(15), resp(16), resp(17);
            ...
            for (int i=1;i<NN;i++){
                ...
                */
                ubA2 << Ax.head(22) ,QS,Q2S, bA.block(0,0,3,1),bA2.block(0,0,3,1) ;
                lbA2 << Ax.head(22) ,QI,Q2I, bA.block(0,0,3,1),bA2.block(0,0,3,1) ;

                A2   <<                         A                        ,         zeros38_6   ,
                       Ja_Total.block(0,0,3,16),  zeros16.block(0,0,3,16),      -eye3,zero3    ,
                       Ja_Total.block(6,0,3,16),  zeros16.block(0,0,3,16),      zero3,-eye3    ;
...
private:
    ...
    double Kwh,Krot;
public:
    Simulador(VectorXd Q, VectorXd Q2);
    VectorXd connect(VectorXd,VectorXd,VectorXd,VectorXd);
...
Simulador::Simulador(VectorXd Q,VectorXd Q2){
    ...
    for(int j=0; j<=300000; j++){
     ...
     resp << prismaticos[1], prismaticos[0], baseRot[2], prismaticos2[1], prismaticos2[0], baseRot2[2],
             ...
             desired2[0], desired2[1], desired2[2];

}

VectorXd Simulador::connect(VectorXd Q,VectorXd Q2,VectorXd QDot,VectorXd Q2Dot){
